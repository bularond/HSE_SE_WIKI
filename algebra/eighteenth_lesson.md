...
- Это подгрпуппа в $(\Z, +)$
- $a := m \cdot k$  $\in m\Z$ 
	пусть $r := n$ - произвольное целое ($=\forall r \in \Z$) $\implies a \cdot r = r \cdot a = m \cdot k \cdot n \in m\Z$
...
**Определение** Идеал $I$ называется **главным**, если $\exists a \in K: I = \{r \cdot a \mid r \in K\}$. Говорят, что идеал $I$ порожден $a$
